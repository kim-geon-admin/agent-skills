...
description: Use when the user asks to inspect Hermes agent profiles, profile health, authentication, gateway status, Google Workspace CLI/GWS authentication, OpenAI Codex connection, Codex runtime, or Korean requests such as 프로필 점검, 프로필 인증 체크, 프로필명 점검, 프로필명 체크, 인증 확인, 연결 확인.
---
...
| Google Workspace CLI 인증 | `gws auth status` | 사용자가 `GWS`, `google workspace cli`, Google Workspace 인증을 말하면 실제 `gws` CLI의 자체 인증 상태를 우선 확인한다. `hermes -p <profile> auth status google-workspace-cli`는 Hermes auth-provider 등록 상태일 수 있으므로, Gmail/Calendar/Tasks 등 Workspace API 사용 가능 여부 판단의 단독 근거로 쓰지 않는다. |
| Gmail 읽기 | `gws gmail users getProfile --params '{"userId":"me"}'` | Gmail 조회 요청이 가능해야 하면 실제 API 호출로 확인한다. scope에 `gmail.readonly`가 있거나 getProfile/list/read가 성공하면 `정상`. |
| Calendar 일정 | `gws calendar calendarList list --params '{"maxResults":3}'` | 일정 조회/관리 요청이 가능해야 하면 실제 Calendar API 호출로 확인한다. scope에 `https://www.googleapis.com/auth/calendar`가 있고 calendarList/events 조회가 성공하면 `정상`. |
| Google Tasks 할 일 | `gws tasks tasklists list --params '{"maxResults":3}'` | 할 일 조회 요청이 가능해야 하면 실제 Tasks API 호출로 확인한다. scope에 `https://www.googleapis.com/auth/tasks.readonly` 또는 tasks scope가 있고 tasklists/tasks 조회가 성공하면 `정상`. 쓰기 요청은 `tasks` 전체 scope가 필요할 수 있으므로 read-only scope만 있으면 쓰기는 `주의`. |
...
| Google Workspace CLI 인증 | `gws auth status`, `GOOGLE_WORKSPACE_CLI_CONFIG_DIR`, `~/.config/gws/client_secret.json`, `~/.config/gws/credentials.enc`, `~/.config/gws/credentials.json`, 또는 `auth/google_oauth.json` | `gws auth status`가 `auth_method`, `has_refresh_token`, `token_valid`, `user`, `scope_count`를 정상으로 반환하면 실제 GWS CLI 사용은 `정상`으로 본다. Hermes auth-provider가 logged out이어도 `gws` 자체 인증이 정상이면 Gmail/Calendar/Tasks 요청은 성공할 수 있다. scope별로 `gmail.readonly`, `calendar`, `tasks.readonly`/`tasks`를 확인하고, 가능하면 실제 read-only API 호출(`gmail users getProfile`, `calendar calendarList list`, `tasks tasklists list`)로 검증한다. 만료됐지만 refresh token이 있으면 `주의`. read-only scope만 있으면 조회는 `정상`, 생성·수정·삭제는 `주의` 또는 `확인 필요`로 분리한다. |
...
조치는 가능한 한 Hermes CLI 또는 실제 사용 CLI 명령으로 제시한다. 예: `gws auth login`, `gws auth setup`, `hermes -p <profile> auth add openai-codex`, `hermes -p <profile> gateway start`, `hermes -p <profile> setup`. Google Workspace는 Hermes auth-provider보다 `gws auth status`와 실제 `gws <service> ...` 호출 결과를 우선한다.
