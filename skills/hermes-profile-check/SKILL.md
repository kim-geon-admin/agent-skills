...
| Google Workspace CLI 인증 | `hermes -p <profile> auth status google-workspace-cli` | 사용자가 `GWS`, `google workspace cli`, Google Workspace 인증을 말하면 이 항목으로 해석한다. |
...
| Google Workspace CLI 인증 | Google Workspace CLI 인증 저장소 또는 `auth/google_oauth.json` | `access`, `refresh`, `expires`, `email` 존재 여부만 본다. 만료됐지만 refresh token이 있으면 `주의`. |
...
조치는 가능한 한 Hermes CLI 명령으로 제시한다. 예: `hermes -p <profile> auth add google-workspace-cli`, `hermes -p <profile> auth add openai-codex`, `hermes -p <profile> gateway start`, `hermes -p <profile> setup`.
